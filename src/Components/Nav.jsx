...
import { fetchBlogPosts } from "../Components/api";
...
const Nav = ({ sentences }) => {
  ...
  return (
    <section className="shadow-md ">
          ...
          <ul className="flex items-center justify-between">
            ...
            <li className="flex items-center">
              ...
              <span className="font-[200] mx-5 m-2 text-sm">
                <div
                  className={`transition-transform ease-out text-black duration-100 ${
                    isAutoChanging && sentences.length > 0 // Check if sentences is defined and not empty
                      ? "translate-x-40 opacity-0"
                      : "-translate-x-0 opacity-100"
                  }`}
                >
                  {sentences.length > 0 ? sentences[currentText] : ""}
                </div>
              </span>
            </li>
      ...
      <header className="bg-white mb-4  justify-between px-10 hidden md:visible md:flex">
        <ul className="flex items-center">
          ...
          <li className="md:hidden text-black lg:flex item-center text-xs font-normal">
            {" "}
            {formattedDate}
          </li>
        </ul>
        ...
        >
          <img
            className="md:w-80 lg:w-96 2xl:w-full h-10 object-contain"
        ...
        <div className="flex items-center">
          ...
          >
            <div class="aspect-ratio-square">
              <img src="images/icons.avif" alt="shopping-cart" />
            </div>
            Store
          </a>
        ...
        <nav className="relative pt-5  mx-5 items-center bg-white">
          <div className="md:hidden flex justify-between  ">
            ...
            <a
              aria-label="search"
              href="/"
              className=" text-neutral-500 flex cursor-pointer "
            >
              <svg
...
export async function getServerSideProps() {
  try {
    // Fetch data for the Nav component
    const posts = await fetchBlogPosts();
    const initialSentences = posts
      .slice(0, 5) // Get the first 5 posts
